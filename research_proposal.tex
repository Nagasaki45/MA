...
In each experiment block the participants listen to music in their headphones, using their Android device and pre-installed application, and interact with other participants and system components in the `silent disco' party.

\todo[inline]{Again, rewrite after better measurements subsection}

Meanwhile, during each experiment block, measurements \ref{measure:disperse}, \ref{measure:groups}, \ref{measure:audio} and \ref{measure:system} are collected.
Before each experiment block and after the last block each participant fill the social interaction survey (measurement \ref{measure:survey:social}).
...
Whereas participants of group $B$ heard the experiment blocks: Control $\rightarrow$ Interactive $\rightarrow$ Control $\rightarrow$ Interactive.

The procedure described above presents several advantages in assessing the main research question compared to experiment 1 design and variants of A / B designs:
