...
By translating physical properties into sound, the Arduino platform opened the door for musicians to use more then just audio and MIDI to communicate with music creation software in interactive environment.
...
For example, AutoRap turns speech into rap by slicing the syllables and mapping them according to different beat styles\footnote{Smules' AutoRap: \href{https://play.google.com/store/apps/details?id=com.smule.autorap}{https://play.google.com/store/apps/details?id=com.smule.autorap}.}.
...
An early attempt to answer this question is found in Darwin's \textit{``Descent of Man''}, with the suggestion that music exist to support sexual selection.
...
According to Brown, music's functionality can be described with regard to both within-group cooperation, promoting group identity and cohesion, as well as between-group competitiveness.
...
On the other hand, IPS has not yet been standardized, and therefore it is unavailable to the average user (\cite{web:turetsky}).
...
The system I  developed --- the Bluetooth Based Relative Indoor Positioning (BBRIP) system --- consists of a nuumber of Bluetooth beacons placed inside bundles of balloons, and an Android application.
...
The received signal strength indication (RSSI) is used as an estimation of the distance between the user and the beacon.
...
\begin{tabularx}{\textwidth}{p{2in} | Y Y Y Y Y }
	...
	\hline
	In the last experiment block I interacted with people I did not knew before the experiment & 1 & 2 & 3 & 4 & 5 \\
\end{tabularx}
