...
\todo[inline]{Check how to ref URLs according to the ``journal of computer music''}
...
% use Huron 2001
From another point of view, in order to understand the origins of music as a common human behavior researches suggest that music might be an evolutionary adaptation.
Supportive evident to that argue could be found by the fact that human have neurological specialization for music processing.
...
First attempt to answer the above question could be found in Darwin's \textit{``Descent of Man''}, suggested that music exist to support sexual selection.
Recent studies criticize this attitude by standing on some fundamental features of music as isometric rhythm and discrete pitches that emphasize musics' synchronization characteristics at the group level, rather then in the individual level (\cite{Brown2000}).
...
\noriinline{Describe the system in present time. `will' $\rightarrow$ `is'}
...
\noriinline{Remove variance from the appendix. Add it to the main text different name because it is not really `variance', maybe `disperse value'}

Usually, the variance of a discrete random variable $X$ is computed as:
\begin{equation*}
	Var(X) = \sum_{i=1}^n \frac{(x_i - \mu)^2}{n}
\end{equation*}
Where $\mu$ is the variable mean value.

The variance of a two dimensional discrete random variable $X$ will be defined using the euclidean distance between two points on the two dimensional plane as:
\begin{equation*}
	Var(X) = \sum_{i=1}^n \frac{dist(x_i, \mu)^2}{n}
\end{equation*}
