...
\noriinline{Here you need an introduction paragraph say that the origins of this research rely on interdisciplinary research: IMS,interactive, social, positioning systems}

\subsection{The origins of IMS}

\noriinline{Say that IMS merges development in several threads: early music software, standartization (MIDI), makers comunity (arduino,internet of things)...}
...
\nori{On another direction:} In the nineteen-eighties, a group of musical instruments manufacturers agreed on universal standard for sending and receiving musical information digitally, establishing the MIDI protocol (\cite{web:quinn}).
...
\noriinline{"Makers" culture - add a pargraph about the phenomena}
...
As examples one can find the works of Chris Milk and Aaron Koblin \nori{Describe one work at least}(\citeauthor{web:milk1}; \citeauthor{web:milk2}) and the startup Interlude which driven by equivalent concept (\citeauthor{web:interlude}).
...
\noriinline{Some references more general about the cogntive and social asspects? sounds to me quite shallow}
...
% \noriinline{Start with "figure xx shows the general architecture of the system" provide clear figures and follow and explain them in the text, this will be more concise and easy to write than abstract prose}
...
% Figure 1 shows a schematic diagram of our system, which consists of a mobile application developed for Android OS [13] and six specially designed Bluetooth beacons [14]. Participants can freely move the beacons that are installed on physical objects, thereby changing the structure of the music in the virtual space. The Android application repeatedly searches for nearby Bluetooth beacons (see figure 1, BB 1 -BB 6 ).
% Received signal strength indication (RSSI) is used as an estimation of the distance between the user and the beacon. This is then sent to a Pure Data (Pd) patch [15] through the libpd API [16]. The Pd patch plays an audio loop corresponding to the nearby beacon. Every audio loop, played by the sound zone players SZP 1 -SZP 6 , is identified by a distinct musical style, and uses the RSSI value differently; for example by controlling volume, filter and granularity of the sound zone. In addition, the different sound zones are modules which can be rhythmically and harmonically synchronized with each other in almost endless combinations.
...
\begin{enumerate}
	...
	In addition, we can define the \definition{momentary disperse} of each group as the variance of its participants' positionings around the center\footnote{Two dimensional variance definition is presented in the appendix.}\todo{write 2d var}.
...
\noriinline{Some general ending paragraph}

\section{Appendix}

\noriinline{Yeh, all the details of the planned experiment should be there}
