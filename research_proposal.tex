...
\noriinline{Refs to sites should be standardized}
...
Traditionally, IMS merge development in several different threads such as music oriented softwares and programming languages (e.g.\ MUSIC-N and Max/MSP), standardization of technologies (e.g.\ MIDI and VST) and the recent emergence of ``makers culture'' (\todo{ref}), to facilitate new ways of music creation.
...
As opposed to early programming languages as GROOVE or the MUSIC-N series, most of those personal computer based languages still exist today and keep progressing (\todo{ref last max/msp version}).
Moreover, new music oriented programing languages continue to be developed, presenting features like ``on-the-fly'' programming (\citeauthor{web:chuck}), web capabilities (\todo{web audio api}) or modular, multi-touch environment for live performance (\citeauthor{web:usine}).
...
A notable example are the works by Chris Milk and Aaron Koblin, in which the video clip is run on a dedicated web page and response to the users by tracking their mouse, keyboard strokes or other inputs, can serve as great examples (\citeauthor{web:milk1}; \citeauthor{web:milk2}; \todo{add beck360 ref}).
...
\noriinline{What about NFCC? Bluetooth 4? say that these is exactly where the market moves today. YOU ALSO NEED HERE SOME EXAMPLES OF SYSTEM AND TECHNOLOGIES OF THIS SORT.}
...
\noriinline{Provide some examples of AR systems}
...
\todo[inline]{This section will be filled with short explanation regarding the motivation for the evaluation and measurements reasoning. It will reference the next section of preliminary results and detailed experiment design.}
...
\todo[inline]{Before refactoring the following paragraph was the last one of the proposal, now it should be removed and another final paragraph should be added at the end.}
...
\todo[inline]{Reasoning for doing another experiment. What insights are necessary that doesn't found in the pilot.}

\subsection{Future experiment}\label{methods:evaluation}

\noriinline{change this intro after section refactoring}
